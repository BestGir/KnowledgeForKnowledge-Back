...
public class LoginCommandHandlerTests
{
    ...
    [Fact]
    public async Task Handle_RequireTelegramOtp_ResolvesStartTokenAndReturnsOtp()
    {
        var ctx = TestDbContext.Create();
        var account = Fakes.Account(telegramId: null);
        account.TelegramLinkToken = "LINK1234";
        ctx.Accounts.Add(account);
        await ctx.SaveChangesAsync();
        ...
        var telegram = new Mock<ITelegramService>();
        telegram.Setup(t => t.TryResolveChatIdByStartTokenAsync("LINK1234", It.IsAny<CancellationToken>()))
            .ReturnsAsync("784967113");
        telegram.Setup(t => t.SendOtpAsync("784967113", It.IsAny<string>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);

        var handler = CreateHandler(ctx, hasher.Object, telegram.Object);
        var result = await handler.Handle(new LoginCommand(account.Login, "pass", RequireTelegramOtp: true), CancellationToken.None);

        result.RequiresOtp.Should().BeTrue();
        result.RequiresTelegramLink.Should().BeFalse();
        result.SessionId.Should().NotBeNullOrWhiteSpace();

        var updated = await ctx.Accounts.FindAsync(account.AccountID);
        updated!.TelegramID.Should().Be("784967113");
        updated.TelegramLinkToken.Should().BeNull();
        telegram.Verify(t => t.TryResolveChatIdByStartTokenAsync("LINK1234", It.IsAny<CancellationToken>()), Times.Once);
        telegram.Verify(t => t.SendOtpAsync("784967113", It.IsAny<string>(), It.IsAny<CancellationToken>()), Times.Once);
    }
}

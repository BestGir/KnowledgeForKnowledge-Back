...
public class CreateReviewCommandHandlerTests
{
    private static CreateReviewCommandHandler CreateHandler(
        ...
        => new(ctx, telegram ?? Mock.Of<ITelegramService>());
    ...
    [Fact]
    public async Task Handle_ReviewCreated_CreatesNotificationAndSendsTelegramToTarget()
    {
        var ctx = TestDbContext.Create();
        var initiator = Fakes.Account(login: "initiator@test.com", telegramId: "initiator_tg");
        var partner = Fakes.Account(login: "partner@test.com", telegramId: "partner_tg");
        ctx.Accounts.AddRange(initiator, partner);
        var deal = Fakes.Deal(initiatorId: initiator.AccountID, partnerId: partner.AccountID, status: DealStatus.Completed);
        ctx.Deals.Add(deal);
        await ctx.SaveChangesAsync();
        ...
        var reviewId = await handler.Handle(
            new CreateReviewCommand(deal.DealID, initiator.AccountID, 5, "Отлично помог"),
            CancellationToken.None);

        var notification = ctx.Notifications.SingleOrDefault(n => n.RelatedEntityId == reviewId);
        notification.Should().NotBeNull();
        notification!.AccountID.Should().Be(partner.AccountID);
        notification.Type.Should().Be(NotificationType.NewReview);
        notification.Message.Should().Contain("5/5");

        telegram.Verify(
            t => t.SendMessageAsync("partner_tg", It.Is<string>(text => text.Contains("5/5")), It.IsAny<CancellationToken>()),
            Times.Once);
    }
}
